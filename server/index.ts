...
app.use('/api/technical-issue', express.json({ limit: '12mb', verify: captureRawBody }));
...
(async () => {
  try {
    ...
    // Ensure password-reset table exists and bootstrap SMTP/payment from .env when DB is empty
    try {
      await ensurePasswordResetTokensTable();
      await bootstrapPlatformEmailFromEnv();
      await bootstrapPlatformStripeFromEnv();
    } catch (bootstrapError) {
      console.error('[INIT] Platform services bootstrap failed:', bootstrapError);
    }
