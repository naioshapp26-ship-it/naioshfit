...
import { getBundleDeleteHasTransactionsMessage, getInsufficientCreditsMessage, getRequestLanguage } from './utils/i18n';
...
export function registerCreditBillingRoutes(app: Express, deps: { isAuthenticated: any }) {
  ...
  app.post('/api/credits/purchase-session', auth, async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json({ message: 'Failed to create purchase session' });
    }
  });
