...
function getEnvEmailConfig(): EmailSendConfig | null {
  ...
  const from = normalizeString(process.env.SMTP_FROM) || user;
  const to = normalizeString(process.env.SMTP_TO);
  ...
  return { host, port, user, pass, from, to, useTls };
}
...
export async function getEmailConfigForScope(context?: Request | EmailScopeContext): Promise<EmailSendConfig | null> {
  ...
  if (tenantPool) {
    const tenantConfig = await getTenantEmailConfig(tenantPool);
...
/**
 * Seed platform_email_settings from .env when the DB row is missing.
 */
export async function bootstrapPlatformEmailFromEnv(): Promise<void> {
  ...
  try {
    const pool = getCentralPool();
    await ensurePlatformEmailSettingsTable(pool);
    const existing = await mapSafeSettings(pool, 'platform_email_settings');
    if (existing?.configured) return;

    await savePlatformEmailSettings({
      smtp_host: envConfig.host,
      smtp_port: envConfig.port,
      smtp_user: envConfig.user,
      smtp_pass: envConfig.pass,
      smtp_from: envConfig.from,
      smtp_to: envConfig.to,
      use_tls: envConfig.useTls,
    });
    console.log('[INIT] Bootstrapped platform email/SMTP settings from environment variables');
  } catch (error) {
    console.error('[INIT] Failed to bootstrap email settings from env:', error);
  }
}
