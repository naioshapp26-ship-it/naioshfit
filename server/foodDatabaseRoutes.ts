...
import { filterFoodsByReligion, isFoodForbiddenForReligion } from '@shared/foodReligionFilter';
...
function duplicateFoodMessage(existing: FoodItem, language: 'en' | 'ar'): string {
  const label = language === 'ar'
    ? (existing.nameAr || existing.name)
    : (existing.name || existing.nameAr);
  if (language === 'ar') {
    return `هذا الطعام موجود بالفعل في قاعدة البيانات: ${label}`;
  }
  return `This food item already exists in the database: ${label}`;
}
...
foodRouter.get('/items', async (req, res) => {
  try {
    ...
    const userReligion = (req.user as any)?.religion ?? null;
    res.json(filterFoodsByReligion(items, userReligion));
  } catch (error) {
...
// Delete a food item (creator or admin)
foodRouter.delete('/items/:id', async (req, res) => {
  try {
    ...
    const database = resolveDb(req);
    const existing = await getFoodById(id, database);
    if (!existing) {
      return res.status(404).json({ message: 'Food item not found' });
    }
    ...
    const isCreator = userId && existing.createdBy === userId;

    if (!isAdmin && !isCreator) {
      return res.status(403).json({ message: 'You can only delete food items you created' });
    }

    await deleteFood(id, database);
...
foodRouter.post('/items', async (req, res) => {
  try {
    ...
    if (isFoodForbiddenForReligion(validatedData, userReligion)) {
      return res.status(400).json({
        message: 'This food item is not allowed for your dietary preferences',
        code: 'FOOD_RELIGION_RESTRICTED',
      });
    }

    const duplicate = await findDuplicateFood(validatedData, database);
    if (duplicate) {
      const language = getRequestLanguage(req);
      return res.status(409).json({
        message: duplicateFoodMessage(duplicate, language),
        code: 'FOOD_DUPLICATE',
