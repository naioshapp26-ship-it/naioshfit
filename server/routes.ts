...
import { storePasswordResetToken, ensurePasswordResetTokensTable } from "./lib/passwordResetTokens";
import { mealDayRange } from "@shared/appTimezone";
import {
...
import { resolveTenantPoolFromRequest } from "./resolveRequestDb";
...
import { getInsufficientCreditsMessage, getLoginErrorMessage, getRequestLanguage } from "./utils/i18n";
...
async function lookupUserByEmailOrUsername(
  ...
  const byEmail = await storage.getUserByEmail(normalized);
  if (byEmail) return byEmail;
  return (await storage.getUserByUsername(normalized)) ?? null;
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const resolveTenantPoolFromSession = async (req: Request) => {
    return resolveTenantPoolFromRequest(req);
  };

  const normalizePinDigits = (value: unknown) => String(value ?? '').replace(/\D/g, '');

  const assertPinNumberAvailable = async (
    pinDigits: string,
    tenantPool?: any,
  ): Promise<{ ok: true; pinDigits: string } | { ok: false; status: number; message: string }> => {
    if (!pinDigits) return { ok: true, pinDigits: '' };
    if (pinDigits.length !== 4) {
      return { ok: false, status: 400, message: 'رقم التحقق يجب أن يكون 4 أرقام بالضبط' };
    }
    if (tenantPool) {
      const result = await tenantPool.query('SELECT id FROM users WHERE pin_number = $1 LIMIT 1', [pinDigits]);
      if (result.rows.length > 0) {
        return { ok: false, status: 400, message: 'رمز التحقق مستخدم بالفعل. اختر رقمًا آخر.' };
      }
    } else {
      const existing = await storage.getUserByPinNumber(pinDigits);
      if (existing) {
        return { ok: false, status: 400, message: 'رمز التحقق مستخدم بالفعل. اختر رقمًا آخر.' };
      }
    }
    return { ok: true, pinDigits };
  };
  ...
  const getMealsByDateForRequest = async (req: any, userId: number, date: Date) => {
    ...
    const { start, end } = mealDayRange(date);
  ...
  const createUserPlanForRequest = async (req: any, plan: schema.InsertUserPlan) => {
    if (!isTenantRequest(req)) {
  ...
  const calculateNutritionMacrosFromWeight = (weight: number) => {
  ...
  const planHasNutritionGoals = (goals: unknown) => {
    if (!goals || typeof goals !== 'object') return false;
    const g = goals as Record<string, unknown>;
    return g.calories !== undefined || g.protein !== undefined ||
      g.carbs !== undefined || g.fat !== undefined ||
      (Array.isArray(g.meals) && g.meals.length > 0);
  };

  const syncNutritionPlanMacrosForWeightChange = async (req: any, userId: number, newWeight: number) => {
    const plan = await getLatestUserPlanForRequest(req, userId);
    if (!plan) return;

    const goals = safeParseJSON(plan.goals) || {};
    if (!planHasNutritionGoals(goals)) return;

    const macros = calculateNutritionMacrosFromWeight(newWeight);
    const updatedGoals = { ...(goals as object), ...macros };
    const updatedPlan = await updateUserPlanForRequest(req, plan.id, { goals: updatedGoals });
    if (updatedPlan) {
      await updateUserDailyStatsGoalsForRequest(req, userId, { ...updatedPlan, goals: updatedGoals });
    }
  };

  const buildLocalNutritionFallbackPlan = (targetUserId: number, coachId: number, targetUser: any) => {
    const { calories, protein, carbs, fat } = calculateNutritionMacrosFromWeight(Number(targetUser?.weight) || 70);
    return insertUserPlanSchema.parse({
      ...
      goals: {
        ...
        tips: ['Hydrate 2-3L/day', 'Prioritize whole foods'],
      },
    });
  };

  const buildLocalWorkoutFallbackPlan = (targetUserId: number, coachId: number, targetUser: any) => {
    const workoutDays = Number(targetUser?.trainingDaysPerWeek) || 3;
    ...
    return insertUserPlanSchema.parse({
      ...
      goals: {
        workoutDays: scheduleDays.length,
        workoutDuration: duration,
        tips: ['Warm-up 5-10 min', 'Cool down and stretch'],
      },
      weeklySchedule: { focus: 'Full Body', workouts },
    });
  };

  const createLocalFallbackBothPlans = async (req: any, targetUserId: number, coachId: number, targetUser: any) => {
    const nutritionPlan = buildLocalNutritionFallbackPlan(targetUserId, coachId, targetUser);
    const workoutPlan = buildLocalWorkoutFallbackPlan(targetUserId, coachId, targetUser);
    const createdNutrition = await createUserPlanForRequest(req, nutritionPlan);
    const workoutGoals = (workoutPlan.goals || {}) as any;
    const existingGoals = (createdNutrition.goals || {}) as any;
    const updatedGoals = {
      ...existingGoals,
      workoutDays: workoutGoals.workoutDays,
      workoutDuration: workoutGoals.workoutDuration,
      tips: [
        ...(Array.isArray(existingGoals.tips) ? existingGoals.tips : []),
        ...(Array.isArray(workoutGoals.tips) ? workoutGoals.tips : []),
      ],
    };
    const updated = await updateUserPlanForRequest(req, createdNutrition.id, {
      title: `${createdNutrition.title} + ${workoutPlan.title}`,
      description: `${createdNutrition.description}\n\nWorkout: ${workoutPlan.description}`,
      weeklyFocus: workoutPlan.weeklyFocus || createdNutrition.weeklyFocus,
      goals: updatedGoals,
      weeklySchedule: workoutPlan.weeklySchedule,
    });
    return { nutrition: updated, workout: updated };
  };
        ...
        try {
          ...
          try {
            ...
            if (tenantPool) {
              ...
              console.log('[TENANT AUTH] Looking up user in tenant database:', { email: loginId });
              ...
              user = await storage.getUserByEmail(loginId);
              
              // Fallback to username if email not found (for backwards compatibility)
              if (!user) {
                user = await storage.getUserByUsername(loginId);
              }
            }
          } catch (dbErr) {
  ...
  const COACH_TRAINEE_CONTACT_FIELDS = new Set(['whatsappWithCode', 'phoneNumber']);
  const TRAINEE_IDENTITY_FIELDS = new Set([
    'firstName', 'lastName', 'email', 'username', 'pinNumber',
    'first_name', 'last_name', 'pin_number',
  ]);

  const coachRequestedIdentityFields = (body: Record<string, unknown>) =>
    Object.keys(body).filter((key) => TRAINEE_IDENTITY_FIELDS.has(key));

  const filterCoachTraineeContactUpdate = (body: Record<string, any>) => {
    const updateData: Record<string, any> = {};
    for (const [key, value] of Object.entries(body)) {
      if (!COACH_TRAINEE_CONTACT_FIELDS.has(key)) continue;
      if (value === '') {
        updateData[key] = null;
        continue;
      }
  ...
  let parsedData: InsertUser;
      ...
      const pinCheck = await assertPinNumberAvailable(
        normalizePinDigits(parsedData.pinNumber),
        isTenantRequest ? tenantPool : undefined,
      );
      if (!pinCheck.ok) {
        return res.status(pinCheck.status).json({ message: pinCheck.message });
      }
      ...
      const userDataWithApproval: InsertUser = {
        ...
        ...(pinCheck.pinDigits ? { pinNumber: pinCheck.pinDigits } : {}),
  ...
  app.get("/api/auth/demo-users", async (req: Request, res: Response) => {
    try {
      ...
      const formattedUsers = demoUsers.map(user => {
        ...
        const loginEmail = user.email?.trim() || `${user.username}@demo.naioshfit.com`;
      ...
      // Backfill missing demo emails/passwords so quick-login works on existing databases
      for (const user of demoUsers) {
        if (!user.username) continue;
        const loginEmail = user.email?.trim() || `${user.username}@demo.naioshfit.com`;
        const patch: Record<string, string> = {};
        if (!user.email?.trim()) patch.email = loginEmail;

        let passwordMatchesDemo = false;
        if (user.password) {
          try {
            passwordMatchesDemo = await bcrypt.compare(DEMO_ACCOUNT_PASSWORD, user.password);
          } catch {
            passwordMatchesDemo = false;
          }
        }
        if (!passwordMatchesDemo) {
          patch.password = await bcrypt.hash(DEMO_ACCOUNT_PASSWORD, 10);
        }
  ...
  app.post("/api/auth/login", (req, res, next) => {
    try {
      const language = getRequestLanguage(req, 'ar');
      passport.authenticate("local", (err: any, user: any, info: any) => {
        if (err) {
          console.error('Passport authentication error:', err);
          return res.status(500).json({ message: getLoginErrorMessage('Authentication internal error', language) });
        }
        if (!user) {
          const errorMessage = getLoginErrorMessage(info?.message, language);
        ...
        req.logIn(user, async (loginErr) => {
          if (loginErr) {
            ...
            return res.status(500).json({ message: getLoginErrorMessage('Login session error', language) });
          }
            ...
            req.session.save((saveErr) => {
              if (saveErr) {
                ...
                return res.status(500).json({ message: getLoginErrorMessage('Session persistence error', language) });
              }
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    try {
      ...
      const pinDigits = String(req.body?.pinNumber ?? '').replace(/\D/g, '');
      const newPassword = req.body?.newPassword;

      // Validate required fields
      if (!email || !pinDigits || !newPassword) {
      ...
      user = await lookupUserByEmailOrUsername(email, tenantPool);
      ...
      const storedPin = String(user.pinNumber ?? '').replace(/\D/g, '');
      if (storedPin !== pinDigits) {
  ...
  app.post("/api/auth/request-password-reset", async (req, res) => {
    try {
      ...
      const foundUser = await lookupUserByEmailOrUsername(email, tenantPool);
      if (foundUser) {
        user = {
          id: foundUser.id,
      ...
      if (user) {
        try {
          ...
          await storePasswordResetToken(user.id, tokenHash, expiresAt, tenantPool || null);
          ...
          if (emailSent) {
            ...
            console.error('[RESET-REQUEST] Failed to send password reset email (SMTP may be unconfigured):', { email: user.email });
          }
        } catch (resetErr) {
          console.error('[RESET-REQUEST] Failed to create reset token or send email:', resetErr);
        }
      } else {
  ...
  app.post("/api/auth/confirm-password-reset", async (req, res) => {
    try {
      ...
      if (tenantPool) {
        ...
        try {
          await ensurePasswordResetTokensTable(tenantPool);
          const result = await tenantPool.query(
        ...
        await ensurePasswordResetTokensTable(null);
  ...
  const linkCoachToTraineeForPlans = async (
    req: any,
    coachId: number,
    traineeId: number,
  ): Promise<{ allowed: boolean; trainee?: any; message?: string }> => {
    const tenantPool = req?.tenantPool || await resolveTenantPoolFromSession(req);
    if (tenantPool) req.tenantPool = tenantPool;

    let trainee: any;
    if (tenantPool) {
      const result = await tenantPool.query('SELECT * FROM users WHERE id = $1 LIMIT 1', [traineeId]);
      if (result.rows.length === 0) return { allowed: false, message: 'User not found' };
      trainee = mapTenantUserRow(result.rows[0]);
    } else {
      trainee = await storage.getUser(traineeId);
      if (!trainee) return { allowed: false, message: 'User not found' };
    }

    const traineeRoles = new Set(['user', 'visitor', 'guest']);
    if (!traineeRoles.has(trainee.role)) {
      return { allowed: false, message: 'Coach assignment is only available for regular member accounts' };
    }

    if (trainee.coachId === coachId) {
      return { allowed: true, trainee };
    }

    if (trainee.coachId && trainee.coachId !== coachId) {
      return { allowed: false, message: 'User already has another coach assigned' };
    }

    let coachUser: any;
    if (tenantPool) {
      const coachResult = await tenantPool.query(
        'SELECT id, role, gym_id, is_approved FROM users WHERE id = $1 LIMIT 1',
        [coachId],
      );
      coachUser = coachResult.rows[0] ? mapTenantUserRow(coachResult.rows[0]) : null;
    } else {
      coachUser = await storage.getUser(coachId);
    }

    if (!coachUser || coachUser.role !== 'coach') {
      return { allowed: false, message: 'Invalid coach account' };
    }
    if (coachUser.isApproved === false) {
      return { allowed: false, message: 'Coach account must be approved before assigning trainees' };
    }

    const gymIdToSet = coachUser.gymId ?? null;
    if (tenantPool) {
      const updateResult = await tenantPool.query(
        'UPDATE users SET coach_id = $1, gym_id = $2, updated_at = NOW() WHERE id = $3 RETURNING *',
        [coachId, gymIdToSet, traineeId],
      );
      if (updateResult.rows.length === 0) {
        return { allowed: false, message: 'Failed to assign coach' };
      }
      trainee = mapTenantUserRow(updateResult.rows[0]);
    } else {
      const updated = await storage.updateUser(traineeId, { coachId, gymId: gymIdToSet });
      if (!updated) return { allowed: false, message: 'Failed to assign coach' };
      trainee = updated;
    }
  ...
  // Get coaches endpoint for admin dashboard and coaches
  app.get("/api/coaches", isAuthenticated, async (req, res) => {
    try {
      ...
      const coaches = allUsers.filter((u: any) => u.role === 'coach' && (u.isApproved !== false));
      const sanitizedCoaches = coaches.map((coachUser: any) => {
        const { password, ...userData } = coachUser;
        return userData;
      });
      ...
      if ((currentUser.role === 'admin' || currentUser.role === 'super_admin') || currentUser.role === 'coach') {
        ...
        if (currentUser.role === 'coach') {
          users = users.filter((u: any) => {
            if (u.role === 'admin' || u.role === 'super_admin' || u.role === 'coach' || u.role === 'gym') {
              return false;
            }
            return !u.coachId || u.coachId === currentUser.id;
          });
        }
  ...
  app.patch("/api/users/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      // Coaches may only update contact info for their assigned trainees
      if (currentUser.role === 'coach') {
        if (userId !== currentUser.id && user.coachId !== currentUser.id) {
          return res.status(403).json({ message: 'Forbidden' });
        }
        if (userId !== currentUser.id) {
          const identityAttempts = coachRequestedIdentityFields(rawData);
          if (identityAttempts.length > 0) {
            return res.status(403).json({
              message: 'Coaches cannot change trainee name, email, or login fields',
            });
          }
          const contactUpdate = filterCoachTraineeContactUpdate(rawData);
          if (Object.keys(contactUpdate).length === 0) {
            return res.status(400).json({ message: 'Coaches can only update WhatsApp / phone contact fields' });
          }
          Object.keys(updateData).forEach((key) => delete updateData[key]);
          Object.assign(updateData, contactUpdate);
        }
      }
      ...
      const weightChanged = Object.prototype.hasOwnProperty.call(updateData, 'weight') &&
        updateData.weight !== previousWeight &&
      ...
      if (tenantPool) {
        ...
        if (updateResult.rows.length > 0) {
          ...
          if (weightChanged) {
            try {
              await syncNutritionPlanMacrosForWeightChange(req, userId, updateData.weight);
            } catch (macroErr) {
              console.error('Failed to sync nutrition macros after weight change:', macroErr);
            }
          }
          return res.json(userData);
        } else {
  ...
  app.get("/api/meals", isAuthenticated, async (req, res) => {
    try {
      ...
      if (dateParam) {
        ...
        const { start, end } = mealDayRange(dateParam);
        ...
        try {
          ...
          if (tenantPool) {
            ...
            await tenantPool.query(
              ...
              [progress.weight, userId]
            );
          } else {
            await storage.updateUser(userId, { weight: progress.weight });
          }
          await syncNutritionPlanMacrosForWeightChange(req, userId, progress.weight);
        } catch (weightSyncErr) {
          console.error('Failed to sync weight/macros from progress log:', weightSyncErr);
        }
      }
  ...
  app.post("/api/cart/checkout", isAuthenticated, async (req, res) => {
    try {
      ...
      if (paymentProvider === 'stripe') {
        ...
        return res.status(400).json({ message: `Unsupported payment provider: ${paymentProvider}` });
      }
  ...
  app.post("/api/quick-add-workout", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!userId || !workoutText || Number.isNaN(targetUserId)) {
      ...
      if (!isAdmin) {
        ...
        let targetUser: any;
        if (tenantPool) {
          const result = await tenantPool.query('SELECT id, coach_id FROM users WHERE id = $1 LIMIT 1', [targetUserId]);
          if (result.rows.length === 0) {
            targetUser = null;
          } else {
            const row = result.rows[0];
            targetUser = { id: row.id, coachId: row.coach_id };
          }
        } else {
          targetUser = await storage.getUser(targetUserId);
        }
        if (!targetUser) {
          return res.status(404).json({ message: 'المتدرب غير موجود' });
        }
        if (targetUser.coachId !== currentUser.id) {
          return res.status(403).json({
            message: 'لا يمكن تعيين خطة تمرين إلا لمتدرب معيّن لك. اطلب من الأدمن تعيين المتدرب أولاً.',
          });
        }
      }
  ...
  app.post("/api/user-plans", isAuthenticated, async (req, res) => {
    try {
      ...
      // Only coaches and admins can create plans (nutrition plans are coach-only)
      const isAdmin = currentUser.role === 'admin' || currentUser.role === 'super_admin';
      if (currentUser.role !== "coach" && !isAdmin) {
      ...
      if (planHasNutritionGoals(parsedData.goals) && currentUser.role !== 'coach') {
        return res.status(403).json({ message: "Only coaches can create nutrition plans" });
      }

      if (currentUser.role === 'coach' && parsedData.userId !== currentUser.id) {
        const link = await linkCoachToTraineeForPlans(req, currentUser.id, parsedData.userId);
        if (!link.allowed) {
          return res.status(403).json({ message: link.message || 'Access denied' });
        }
      }
  ...
  app.patch("/api/user-plans/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      const involvesNutrition = planHasNutritionGoals(existingGoals) || planHasNutritionGoals(newGoals);
      const isAdmin = currentUser.role === 'admin' || currentUser.role === 'super_admin';

      if (involvesNutrition) {
        if (currentUser.role !== 'coach' || plan.coachId !== currentUser.id) {
          console.log(`Nutrition plan update denied for user ${currentUser.id} on plan ${planId}`);
          return res.status(403).json({ message: "Only the assigned coach can update nutrition plans" });
        }
      } else if (plan.coachId !== currentUser.id && !isAdmin) {
  ...
  app.delete("/api/user-plans/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      if (planHasNutritionGoals(safeParseJSON(plan.goals))) {
        if (currentUser.role !== 'coach' || plan.coachId !== currentUser.id) {
          return res.status(403).json({ message: "Only the assigned coach can delete nutrition plans" });
        }
      } else if (plan.coachId !== currentUser.id && (currentUser.role !== 'admin' && currentUser.role !== 'super_admin')) {
        return res.status(403).json({ message: "Forbidden" });
      }
  ...
  app.post("/api/coach/ai/generate-plan", isAuthenticated, async (req, res) => {
    ...
    try {
      ...
      const usersDb = resolveDb(req);
      let [targetUser] = await usersDb.select().from(users).where(eq(users.id, targetUserId));
      if (!targetUser) return res.status(404).json({ message: "User not found" });
      
      // Allow users to generate plans for themselves, or coaches to generate for their clients
      const isGeneratingForSelf = currentUser.id === targetUserId;
      let isCoachForUser = currentUser.role === 'coach' && targetUser.coachId === currentUser.id;
      const isAdmin = (currentUser.role === 'admin' || currentUser.role === 'super_admin');

      if (currentUser.role === 'coach' && !isGeneratingForSelf && !isCoachForUser) {
        const link = await linkCoachToTraineeForPlans(req, currentUser.id, targetUserId);
        if (!link.allowed) {
          return res.status(403).json({ message: link.message || "Access denied. You can only generate plans for yourself or your clients." });
        }
        if (link.trainee) {
          targetUser = link.trainee;
          isCoachForUser = true;
        }
      }
      ...
      if (planType === 'nutrition' && isAdmin && !isCoachForUser) {
        return res.status(403).json({ message: "Only coaches can generate nutrition plans for clients." });
      }
...
- Keep output as pure JSON only.`;
  ...
  app.post("/api/coach/ai/generate-both", isAuthenticated, async (req, res) => {
    // Prevent long-lived connections from holding Passenger process slots
    req.setTimeout(90_000);
    res.setTimeout(90_000);
    try {
      const currentUser = req.user as any;
      ...
      if (!isGeneratingForSelf && !isCoachForUser) {
        return res.status(403).json({ message: "Access denied. You can only generate plans for yourself or your clients." });
      }
      ...
      if (!planConfig && allowLocalFallback) {
        const fallbackPlans = await createLocalFallbackBothPlans(req, targetUserId, currentUser.id, targetUser);
        return res.status(201).json({
          ...fallbackPlans,
...
Return JSON with this exact shape for nutrition:\n${nutritionShape}\n\nRules:\n- Numbers must be realistic.\n- Meals must match preferences / restrictions.\n- All user-facing text MUST be in Arabic (title, description, meal names, meal items, tips).\n- Output pure JSON only.`;
  ...
  await attemptWithRetries();
      ...
      if (!results.nutrition && !results.workout) {
        ...
        if (allowLocalFallback) {
          try {
            const fallbackPlans = await createLocalFallbackBothPlans(req, targetUserId, currentUser.id, targetUser);
            return res.status(201).json({ ...fallbackPlans, usedLocalFallback: true });
          } catch (fallbackErr) {
            console.error('Local fallback after AI failure failed:', fallbackErr);
          }
        }
      ...
      if (allowLocalFallback) {
        try {
          ...
          const targetUserId = parseInt(String(userId));
          const currentUser = req.user as any;
          if (!Number.isNaN(targetUserId) && currentUser?.id) {
            const usersDb = resolveDb(req);
          ...
          console.error('Local fallback after generate-both error failed:', fallbackErr);
        }
      }
  ...
  // Legacy coach media certificates (content_library) — do not shadow course certificate routes
  app.get('/api/coach/media-certificates', isAuthenticated, isCoachOrAdmin, async (req, res) => {
    try {
      const currentUser = req.user as any;
      const coachIdParam = req.query.coachId as string | undefined;
      const targetCoachId = (currentUser.role === 'admin' || currentUser.role === 'super_admin') && coachIdParam ? parseInt(coachIdParam) : currentUser.id;
  ...
  app.post('/api/coach/media-certificates', isAuthenticated, isCoachOrAdmin, async (req, res) => {
    try {
      const currentUser = req.user as any;
      const coachId = (currentUser.role === 'admin' || currentUser.role === 'super_admin') && req.body?.coachId ? parseInt(req.body.coachId) : currentUser.id;
      if (Number.isNaN(coachId)) {
  ...
  app.patch('/api/coach/media-certificates/:id', isAuthenticated, isCoachOrAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const currentUser = req.user as any;
      const tenantPool = (req as any).tenantPool || await resolveTenantPoolFromSession(req);
  ...
  app.delete('/api/coach/media-certificates/:id', isAuthenticated, isCoachOrAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const currentUser = req.user as any;
      const tenantPool = (req as any).tenantPool || await resolveTenantPoolFromSession(req);
  ...
  app.patch('/api/coach/users/:id', isAuthenticated, isCoachOrAdmin, async (req, res) => {
    try {
      ...
      const isAdmin = currentUser.role === 'admin' || currentUser.role === 'super_admin';
      const isAssignedCoach = currentUser.role === 'coach' && targetUser.coachId === currentUser.id;
      ...
      // Coaches: contact fields only. Admins on this route: contact fields only (use /api/users/:id for profile edits).
      const allowedFields = COACH_TRAINEE_CONTACT_FIELDS;
      const updateData: Record<string, any> = {};
      Object.entries(rawData).forEach(([key, value]) => {
  ...
  // Assign trainee to a coach (coach can claim unassigned trainees; admin can assign any coach)
  app.patch('/api/coach/users/:id/assign', isAuthenticated, isCoachOrAdmin, async (req, res) => {
    try {
      const targetUserId = parseInt(req.params.id);
      const currentUser = req.user as any;
      const tenantPool = (req as any).tenantPool || await resolveTenantPoolFromSession(req);
      ...
      const traineeRoles = new Set(['user', 'visitor', 'guest']);
      if (!traineeRoles.has(targetUser.role)) {
        return res.status(400).json({ message: 'Coach assignment is only available for regular member accounts' });
      }

      const isAdmin = currentUser.role === 'admin' || currentUser.role === 'super_admin';
      let coachIdToAssign: number | null;

      if (isAdmin) {
        if (!Object.prototype.hasOwnProperty.call(req.body, 'coachId')) {
          return res.status(400).json({ message: 'coachId is required for admin assignment' });
        }
        const rawCoachId = req.body.coachId;
        if (rawCoachId === null || rawCoachId === '' || rawCoachId === 'UNASSIGN') {
          coachIdToAssign = null;
        } else {
          const parsed = typeof rawCoachId === 'number' ? rawCoachId : parseInt(String(rawCoachId), 10);
          if (Number.isNaN(parsed)) {
            return res.status(400).json({ message: 'coachId must be a valid number' });
          }
          coachIdToAssign = parsed;
        }
      } else {
        if (targetUser.coachId && targetUser.coachId !== currentUser.id) {
          return res.status(400).json({ message: 'User already has another coach assigned' });
        }
        if (targetUser.coachId === currentUser.id) {
          const { password, ...userData } = targetUser;
        ...
        if (currentUser.isApproved === false) {
          return res.status(403).json({ message: 'Coach account must be approved before assigning trainees' });
        }
        coachIdToAssign = currentUser.id;
      }

      let gymIdToSet: number | null = null;
      if (coachIdToAssign !== null) {
        let assignedCoach: any;
        if (tenantPool) {
          const coachResult = await tenantPool.query(
            'SELECT id, role, gym_id, is_approved FROM users WHERE id = $1 LIMIT 1',
          ...
          assignedCoach = coachResult.rows[0] ? mapTenantUserRow(coachResult.rows[0]) : null;
        } else {
          assignedCoach = await storage.getUser(coachIdToAssign);
        }
        if (!assignedCoach || assignedCoach.role !== 'coach') {
          return res.status(400).json({ message: 'coachId must reference an existing coach' });
        }
        if (assignedCoach.isApproved === false) {
          return res.status(400).json({ message: 'Selected coach is not approved' });
        }
        gymIdToSet = assignedCoach.gymId ?? null;
      }

      if (tenantPool) {
        const updateResult = await tenantPool.query(
          'UPDATE users SET coach_id = $1, gym_id = $2, updated_at = NOW() WHERE id = $3 RETURNING *',
          [coachIdToAssign, gymIdToSet, targetUserId]
        );
        if (updateResult.rows.length === 0) return res.status(500).json({ message: 'Failed to assign coach' });
        const { password, ...userData } = mapTenantUserRow(updateResult.rows[0]);
        return res.status(200).json(userData);
      }
      ...
      if (!updatedUser) return res.status(500).json({ message: 'Failed to assign coach' });
      const { password, ...userData } = updatedUser;
      return res.status(200).json(userData);
    } catch (error) {
      console.error('Error assigning coach to user:', error);
      return res.status(500).json({ message: 'Error assigning coach' });
    }
  });
  ...
  app.delete('/api/coach/users/:id', isAuthenticated, isCoachOrAdmin, async (req, res) => {
    try {
      ...
      if (currentUser.role === 'coach') {
        return res.status(403).json({ message: 'Coaches cannot delete user accounts. Use unassign instead.' });
      }
  ...
  app.post("/api/users", isAuthenticated, async (req, res) => {
    try {
      ...
      if (tenantPool) {
        ...
        const username = normalizedIdentity.username || email;
        ...
        // Coaches created by admin are auto-approved; all admin-created users get an active subscription
        const userRole = role || 'user';
        const subscriptionType = '1_month';
        const subscriptionStartDate = new Date();
        const subscriptionEndDate = calculateSubscriptionEndDate(subscriptionStartDate, subscriptionType);
        const isCoachCreatedByAdmin = userRole === 'coach';
        ...
        const pinCheck = await assertPinNumberAvailable(normalizePinDigits(parsedData.pinNumber));
        if (!pinCheck.ok) {
          return res.status(pinCheck.status).json({ message: pinCheck.message });
        }
    ...
    try {
      ...
      if (reportData.screenshot) {
        try {
          ...
          const extMatch = reportData.screenshot.match(/^data:image\/([a-z]+);base64,/i);
          const ext = extMatch?.[1] === 'png' ? 'png' : 'jpg';
          const filename = reportData.screenshotFilename?.trim()
            ? reportData.screenshotFilename.replace(/[^\w.\-]/g, '_')
            : `bug-report-${timestamp}.${ext}`;
      ...
      let reporterUserId = (req.user as any)?.id ?? reportData.reporterUserId ?? null;
      let reporterName = reportData.reporterName || '';
      let reporterUsername = reportData.reporterUsername || '';
      let reporterEmail = reportData.email || '';
      let reporterPhone = reportData.phone || '';

      if (!reporterUserId && reporterEmail) {
        try {
          const byEmail = await storage.getUserByEmail(reporterEmail.trim().toLowerCase());
          if (byEmail?.id) reporterUserId = byEmail.id;
        } catch {}
      }
      ...
      if (reporterUserId) {
        try {
          ...
          let reporter: any;
          if (tenantPool) {
            const result = await tenantPool.query('SELECT * FROM users WHERE id = $1 LIMIT 1', [reporterUserId]);
            reporter = result.rows[0] ? mapTenantUserRow(result.rows[0]) : null;
          } else {
            reporter = await storage.getUser(reporterUserId);
          }
          if (reporter) {
            reporterName = reporterName || `${reporter.firstName || ''} ${reporter.lastName || ''}`.trim();
            reporterUsername = reporterUsername || reporter.username || '';
            reporterEmail = reporterEmail || reporter.email || '';
            reporterPhone = reporterPhone || reporter.whatsappWithCode || reporter.phoneNumber || '';
          }
        } catch (reporterErr) {
          console.error('Failed to resolve reporter user for technical issue:', reporterErr);
        }
      }

      const reporterAccountLabel = [
        reporterName,
      ...
      if (reporterAccountLabel) {
        const accountLine = `[Account: ${reporterAccountLabel}]`;
        if (!description.includes(accountLine)) {
      ...
      const webhookData = {
        ...
        reporterName,
        reporterUsername,
        reporterAccount: reporterAccountLabel || reportData.reporterAccount || '',
      ...
      // Persist report to disk for later triage (backup)
      try {
        const fs = await import('fs');
        const path = await import('path');
        ...
        console.error('Failed to persist technical issue report to disk:', fileErr);
      }

      // Primary store: PostgreSQL (survives deploys; visible in admin panel)
      try {
        await db.insert(userReports).values({
          reportId: webhookData.reportId,
          issueType: webhookData.issueType,
          description: webhookData.description,
          userEmail: webhookData.userEmail || null,
          phoneNumber: webhookData.phoneNumber || null,
          screenshotPath: webhookData.screenshotPath || null,
          pageUrl: webhookData.pageUrl || null,
          userAgent: webhookData.userAgent || null,
          reporterUserId: webhookData.reporterUserId ?? null,
        });
      } catch (dbErr) {
        console.error('Failed to persist technical issue report to database:', dbErr);
      }
  ...
  app.get("/api/admin/technical-issues", isAuthenticated, async (req, res) => {
    try {
      const currentUser = req.user as any;
      if (currentUser.role !== 'admin' && currentUser.role !== 'super_admin') {
        return res.status(403).json({ message: 'Access denied' });
      ...
      const parseAccountFromDescription = (description?: string | null) => {
        if (!description) return '';
        const match = description.match(/^\[Account:\s*(.+?)\](?:\r?\n|$)/);
        return match?.[1]?.trim() || '';
      };
      ...
      const normalizeIssue = (issue: any) => {
        ...
        const parsedAccount = parseAccountFromDescription(issue.description);
        const reporterAccount = issue.reporterAccount
          || [
            reporterName,
            issue.reporterUsername ? `@${issue.reporterUsername}` : '',
            issue.reporterDbEmail || issue.userEmail || '',
          ].filter(Boolean).join(' | ')
          || parsedAccount
          || issue.userEmail
          || '';
        return {
          ...
          userEmail: issue.userEmail || issue.reporterDbEmail || undefined,
          phoneNumber: issue.phoneNumber || undefined,
          screenshotPath: issue.screenshotPath || undefined,
          pageUrl: issue.pageUrl || undefined,
          userAgent: issue.userAgent || undefined,
          reporterUserId: issue.reporterUserId || undefined,
          reporterName: reporterName || parsedAccount || undefined,
          reporterUsername: issue.reporterUsername || undefined,
          reporterAccount: reporterAccount || undefined,
          timestamp: issue.timestamp || (issue.createdAt?.toISOString?.() ?? issue.createdAt),
        };
      };
      ...
      try {
        const rows = await db
          .select({
            ...
            issueType: userReports.issueType,
            description: userReports.description,
            userEmail: userReports.userEmail,
            phoneNumber: userReports.phoneNumber,
            screenshotPath: userReports.screenshotPath,
            pageUrl: userReports.pageUrl,
            userAgent: userReports.userAgent,
            reporterUserId: userReports.reporterUserId,
            createdAt: userReports.createdAt,
            reporterFirstName: users.firstName,
            reporterLastName: users.lastName,
            reporterDbEmail: users.email,
          ...
          .limit(500);
        reports = rows.map((row) => normalizeIssue(row));
      } catch (dbErr) {
        console.error('Error fetching technical issues from database:', dbErr);
      }
      ...
      res.status(500).json({ message: 'Failed to fetch technical issues' });
    }
  });

  app.get('/api/admin/technical-issues/:reportId/screenshot', isAuthenticated, async (req, res) => {
    try {
      const currentUser = req.user as any;
      if (currentUser.role !== 'admin' && currentUser.role !== 'super_admin') {
        return res.status(403).json({ message: 'Access denied' });
      ...
      const [report] = await db
        .select({ screenshotPath: userReports.screenshotPath })
        .from(userReports)
      ...
      const absolutePath = path.join(process.cwd(), report.screenshotPath);
      if (!fs.existsSync(absolutePath)) {
        return res.status(404).json({ message: 'Screenshot file missing' });
      }

      return res.sendFile(absolutePath);
    } catch (error) {
      console.error('Error serving technical issue screenshot:', error);
      return res.status(500).json({ message: 'Failed to load screenshot' });
    }
  });
