...
export async function getPlatformPaymobKeys(): Promise<{
  publicKey: string;
  secretKey: string;
...
export async function getPlatformPaymobPublicConfig(): Promise<{ publicKey: string | null; baseUrl: string | null }> {
  const pool = getCentralPool();
  ...
  try {
    const result = await pool.query(
      'SELECT paymob_public_key, paymob_base_url FROM platform_payment_settings LIMIT 1'
    );
...
export async function isPlatformPaymobConfigured(): Promise<boolean> {
  ...
  try {
    const result = await pool.query(
      'SELECT 1 FROM platform_payment_settings WHERE paymob_public_key IS NOT NULL AND paymob_secret_key IS NOT NULL LIMIT 1'
    );
...
export async function createPlatformPaymobIntention(
...
export async function verifyPlatformPaymobWebhook(
  ...
  const keys = await getPlatformPaymobKeys();
  if (!keys.hmacSecret) {
    console.warn('[PAYMOB] Platform webhook secret not configured - skipping verification (NOT RECOMMENDED for production)');
    return true;
  }
