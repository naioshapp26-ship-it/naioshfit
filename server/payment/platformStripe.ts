...
function normalizeEnvString(value: string | undefined | null): string | null {
...
function getEnvStripeKeys(): {
  ...
  const publishableKey = normalizeEnvString(process.env.STRIPE_PUBLISHABLE_KEY);
  const secretKey = normalizeEnvString(process.env.STRIPE_SECRET_KEY);
  if (!publishableKey || !secretKey) return null;

  const webhookSecret = normalizeEnvString(process.env.STRIPE_WEBHOOK_SECRET);
  const isLiveMode = (process.env.STRIPE_IS_LIVE_MODE ?? 'false').toLowerCase() === 'true';
  return { publishableKey, secretKey, webhookSecret, isLiveMode };
}

/**
 * Seed platform_payment_settings from .env when the DB row is missing.
 * Safe to call on every startup (no-op when DB already configured).
 */
export async function bootstrapPlatformStripeFromEnv(): Promise<void> {
  const envKeys = getEnvStripeKeys();
  if (!envKeys) return;
  ...
  try {
    ...
    const existing = await pool.query(
      `SELECT 1 FROM platform_payment_settings
       WHERE stripe_secret_key IS NOT NULL AND stripe_publishable_key IS NOT NULL
       LIMIT 1`
    );
    if (existing.rows.length > 0) return;

    await savePlatformPaymentSettings({
      stripe_publishable_key: envKeys.publishableKey,
      stripe_secret_key: envKeys.secretKey,
      stripe_webhook_secret: envKeys.webhookSecret || undefined,
      is_live_mode: envKeys.isLiveMode,
    });
    console.log('[INIT] Bootstrapped platform Stripe settings from environment variables');
  } catch (error) {
    console.error('[INIT] Failed to bootstrap Stripe settings from env:', error);
  }
}
...
export async function getPlatformStripeKeys(): Promise<{
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      if (row.stripe_publishable_key && row.stripe_secret_key) {
        return {
    ...
    if (!String(error?.message || '').includes('does not exist')) {
      console.error('[PAYMENT] Failed to read platform Stripe keys from DB:', error);
    }
  }
...
export async function getPlatformPublishableKey(): Promise<string | null> {
  const envKeys = getEnvStripeKeys();
  if (envKeys?.publishableKey) return envKeys.publishableKey;
  ...
  try {
    ...
    return result.rows[0].stripe_publishable_key || null;
  } catch {
    return null;
  }
}
