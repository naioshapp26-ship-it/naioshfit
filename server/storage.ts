...
export interface IStorage {
  ...
  getUserByPinNumber(pinNumber: string): Promise<User | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async getUserByPinNumber(pinNumber: string): Promise<User | undefined> {
    const normalized = pinNumber.replace(/\D/g, '');
    if (!normalized) return undefined;
    return Array.from(this.users.values()).find(
      (user) => String(user.pinNumber ?? '').replace(/\D/g, '') === normalized,
    );
  }
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByPinNumber(pinNumber: string): Promise<User | undefined> {
    try {
      const normalized = pinNumber.replace(/\D/g, '');
      if (!normalized) return undefined;
      const result = await db.select().from(users).where(eq(users.pinNumber, normalized)).limit(1);
      return result[0] || undefined;
    } catch (err) {
      console.error('[DB] getUserByPinNumber failed', { pinNumber, err });
      throw err;
    }
  }
