...
**SaaS tenant env (Railway):** `TENANT_DB_ENCRYPTION_KEY` and `TENANT_DATABASE_URL_TEMPLATE` are **auto-derived** from `SESSION_SECRET` (16+ chars) and `DATABASE_URL` at startup. Set `SAAS_AUTO_TENANT_ENV=0` to disable. On Railway (detected via `RAILWAY_*` env vars or `*.rlwy.net` / `*.railway.internal` in `DATABASE_URL`), tenant isolation defaults to **schema** mode (shared Postgres DB, one schema per tenant — avoids SSL failures when provisioning separate `tenant_*` databases). Override with `SAAS_TENANT_ISOLATION=database`. Verify `/api/setup/status` → `tenantProvisioning.ready: true` and `demoUsers: 4` after first deploy boot (schema bootstrap can take 1–2 minutes).
