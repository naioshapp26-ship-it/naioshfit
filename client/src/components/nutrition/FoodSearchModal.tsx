...
const FoodSearchModal: React.FC<FoodSearchModalProps> = ({ open, onClose, onSelect }) => {
  ...
  const searchWithAI = async (query: string) => {
    try {
      ...
      if (response.status === 409) {
        toast({
          title: t("error") || "Error",
          description: getDuplicateFoodToastDescription(t, language, responseData.existingItem),
          variant: "destructive",
        });
        if (responseData.existingItem) {
          setFilteredItems([responseData.existingItem]);
          setFoodItems((prev) => {
            const exists = prev.some((item) => item.id === responseData.existingItem.id);
            return exists ? prev : [responseData.existingItem, ...prev];
          });
          handleSelectFood(responseData.existingItem);
        }
        return;
      }

      if (!response.ok) {
        throw new Error(responseData?.message || 'Failed to search with AI');
      }

      const newFood = responseData;
  ...
  const handleCreateFood = async () => {
    try {
      ...
      if (!newFoodData.name || !newFoodData.nameAr?.trim() || !newFoodData.calories || !newFoodData.proteins || 
      ...
      if (response.status === 409) {
        ...
        toast({
          title: t("error") || "Error",
          description: getDuplicateFoodToastDescription(t, language, duplicatePayload.existingItem),
          variant: "destructive",
        });
        if (duplicatePayload.existingItem) {
          setSelectedFood(duplicatePayload.existingItem);
          setQuantity(duplicatePayload.existingItem.servingSizeGrams || 100);
        }
        setLoading(false);
        return;
      }
  ...
  const handleDeleteFood = async (food: FoodItem, event: React.MouseEvent) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/food-database/items/${food.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload?.message || 'Failed to delete food item');
      }
      ...
      toast({
        title: t("success") || "Success",
        description: t("foodDeletedSuccessfully"),
      });
    } catch (error) {
      toast({
        title: t("error") || "Error",
      ...
      <DialogContent className="sm:max-w-[800px] max-w-[95vw] max-h-[90vh] h-[90vh] flex flex-col w-full mx-4">
        ...
        {showCreateForm && (
          <div className="mb-4 p-4 border rounded-lg bg-gray-50">
            <h3 className="font-medium mb-3">{t("createNewFood")}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <FoodCategorySelect
                  ...
                  onChange={(value) => setNewFoodData({ ...newFoodData, category: value })}
                />
              </div>
              <div>
