...
export function AdminUsersTable({
  ...
  const formatSubscriptionPlan = (user: any) => {
    if (!user.subscriptionType || user.subscriptionType === 'free') {
      return t('courseFree');
    }
    const match = String(user.subscriptionType).match(/^(\d+(?:\.\d+)?)(?:_months?)?$/);
    if (match) {
      const months = parseFloat(match[1]);
      const template = months === 1 ? t('monthPlan') : t('monthsPlan');
      return template.replace('{months}', String(months));
    }
    return String(user.subscriptionType).replace(/_/g, ' ');
  };

  const canAssignCoachToUser = (user: any) => {
    const role = user?.role;
    return role === 'user' || role === 'visitor' || role === 'guest';
  };
  ...
  const getStatusBadge = (user: any) => {
    ...
    switch (status) {
      case 'active':
        return { label: t('active'), className: 'bg-green-100 text-green-800 border-green-300' };
      case 'expired':
        return { label: t('userStatusExpired'), className: 'bg-red-100 text-red-800 border-red-300' };
      case 'suspended':
        return { label: t('suspended'), className: 'bg-orange-100 text-orange-800 border-orange-300' };
      case 'none':
      default:
        return { label: t('none'), className: 'bg-gray-100 text-gray-800 border-gray-300' };
    }
  };
