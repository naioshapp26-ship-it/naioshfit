...
const formSchema = insertCourseSchema.omit({ instructorId: true }).extend({
...
export function CourseForm({ course, onSuccess }: CourseFormProps) {
  ...
  function getDefaultValues(course?: Course | null): FormData {
    return {
      title: course?.title || "",
  ...
  const createMutation = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      if (payload.duration === undefined || Number.isNaN(payload.duration)) {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/coach/courses'] });
      toast({
    ...
    onError: (error: Error) => {
      toast({
        title: t("error"),
        description: error.message || (isEditing ? t("failedToUpdateCourse") : t("failedToCreateCourse")),
        variant: "destructive"
      });
    }
  });
    ...
    <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="title">{t("courseTitle")} (EN) <RequiredMark /></Label>
            <Input
              id="title"
              {...form.register("title")}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-2">
            <Label htmlFor="category">{t("courseCategory")} <RequiredMark /></Label>
          ...
          <div className="space-y-2">
            <Label htmlFor="level">{t("courseLevel")} <RequiredMark /></Label>
