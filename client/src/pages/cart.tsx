...
const CartPage: React.FC = () => {
  ...
  const showPublicHeader = isVisitor;
  ...
  return (
    <section
      className={`p-4 md:p-6 lg:p-8 min-h-screen ${showPublicHeader ? 'bg-gradient-to-b from-slate-200 via-gray-100 to-white dark:from-zinc-900 dark:via-zinc-900 dark:to-zinc-800' : ''} ${isRTL ? 'text-right' : 'text-left'}`}
      dir={isRTL ? 'rtl' : 'ltr'}
    >
      {showPublicHeader && (
        <div className="mb-6">
          <PublicHeader
            ...
            subtitle={t('cartEmptyDescription') || 'Your cart is empty. Start shopping to add items.'}
            backHref="/store"
            backLabel={t('continueShopping') || 'Continue Shopping'}
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            {isVisitor && (
              <p className="text-sm text-gray-500 mb-4">{t('cartLoginHint') || 'Sign in to add products and checkout.'}</p>
            )}
