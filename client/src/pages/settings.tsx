...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    const allowedTabs = new Set(['account', 'notifications', 'billing', 'security']);
  ...
  return (
    ...
    <section className={`p-4 md:p-6 lg:p-8 relative min-h-screen ${isRTL ? 'text-right' : 'text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        {/* Billing Settings */}
        <TabsContent value="billing" className={isRTL ? 'text-right' : 'text-left'}>
          <div className={`grid gap-4 ${isRTL ? 'text-right' : 'text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
            <BillingPanel />
          </div>
        </TabsContent>
