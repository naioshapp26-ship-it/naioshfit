...
const SaasSignupPage = () => {
  ...
  const resolveTenantUrl = (createdTenant: TenantSummary) => {
    if (createdTenant.tenantUrl) {
      return createdTenant.tenantUrl;
    }
    return buildTenantPublicUrl(createdTenant.subdomain, mainDomain, { path: "/auth" });
  };

  const tenantUrl = tenant ? resolveTenantUrl(tenant) : null;

  const redirectToTenant = (createdTenant: TenantSummary) => {
    const url = resolveTenantUrl(createdTenant);
    if (!url) return;
  ...
  const validate = () => {
    ...
    if (!form.subscriptionPlan.trim()) {
      errors.subscriptionPlan = t("saasSubscriptionPlanRequired");
    }
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div className="space-y-2">
                <Label htmlFor="companyName">{t("saasCompanyNameLabel")}<RequiredMark /></Label>
                <Input
                  id="companyName"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="adminName">{t("saasAdminNameLabel")}<RequiredMark /></Label>
                  <Input
                    id="adminName"
                ...
                <div className="space-y-2">
                  <Label htmlFor="adminEmail">{t("saasAdminEmailLabel")}<RequiredMark /></Label>
                  <Input
                    id="adminEmail"
                    type="email"
              ...
              <div className="space-y-2">
                <Label htmlFor="adminPhone">{t("saasAdminPhoneLabel")} ({t("optional")})</Label>
                <Input
                  id="adminPhone"
                  type="tel"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="adminPassword">{t("saasAdminPasswordLabel")}<RequiredMark /></Label>
                ...
                <div className="space-y-2">
                  <Label htmlFor="subscriptionPlan">{t("saasSubscriptionPlanLabel")}<RequiredMark /></Label>
                  <select
                    id="subscriptionPlan"
                    value={form.subscriptionPlan}
                    onChange={(event) => {
                      const value = event.target.value;
                      setForm((prev) => ({ ...prev, subscriptionPlan: value }));
                      if (fieldErrors.subscriptionPlan) {
                        setFieldErrors((prev) => {
                          const next = { ...prev };
                  ...
                  {fieldErrors.subscriptionPlan && (
                    <p className="text-xs text-red-500">{fieldErrors.subscriptionPlan}</p>
                  )}
                </div>
              </div>
