...
export function AdminDashboard() {
  ...
  const assignCoachMutation = useMutation({
    mutationFn: async ({ userId, coachId }: { userId: number, coachId: string | null }) => {
      const response = await apiRequest('PATCH', `/api/users/${userId}`, { coachId: coachId ? parseInt(coachId) : null });
      return response.json();
    },
    onSuccess: (updatedUser) => {
      queryClient.invalidateQueries({ queryKey: ['/api/users'] });
  ...
  const createCategoryMutation = useMutation({
    mutationFn: async (data: typeof categoryFormData) => {
      const response = await apiRequest('POST', '/api/admin/affiliate-categories', data);
      return response.json();
    },
    onSuccess: () => {
      ...
      setCategoryFormData({ nameEn: '', nameAr: '', slug: '', isActive: true, displayOrder: 0 });
  ...
  const updateCategoryMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number, data: typeof categoryFormData }) => {
      const response = await apiRequest('PATCH', `/api/admin/affiliate-categories/${id}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/affiliate-categories'] });
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    if (!userFormData.username || !userFormData.email || !userFormData.password) {
      const errorMsg = language === 'ar' 
        ? "يرجى ملء جميع الحقول المطلوبة (اسم المستخدم، البريد الإلكتروني، وكلمة المرور)" 
        : "Please fill all required fields (username, email, and password)";
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex gap-1">
                                      ...
                                      <Button
                                        ...
                                        title="Assign Me as Coach"
                                        disabled={assignCoachMutation.isPending || foundUser.coachId === user?.id}
                                      >
                    ...
                    <CardHeader>
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            <Plus className="w-4 h-4 sm:mr-1" /> <span>{getTranslation(managingUser.language || 'en', 'add')}</span>
                          </Button>
                          <Button
                            variant="secondary"
                            size="sm"
                            ...
                            onClick={async () => {
                              try {
                                ...
                                await queryClient.invalidateQueries({ queryKey: ['/api/user-plans'] });
                                await queryClient.invalidateQueries({ queryKey: ['/api/user-plans', managingUser.id] });
                                await queryClient.invalidateQueries({ queryKey: ['/api/user-plans', { latest: true }] });
                                toast({ title: getTranslation(managingUser.language || 'en', 'plansCreatedSuccessfully') });
                              } catch (e: any) {
                                let errorMessage = e?.message || 'Please try again';
                                if (e?.name === 'AbortError') {
                                  setTimeout(() => {
                                    queryClient.invalidateQueries({ queryKey: ['/api/user-plans'] });
                                    queryClient.invalidateQueries({ queryKey: ['/api/user-plans', managingUser.id] });
                                  ...
                                  toast({
                                    ...
                                    description: 'This is taking longer than expected. Plans may be saved shortly; we will refresh automatically.',
                                  });
                                  return;
                                }
                            ...
                            <div className="border rounded p-3 bg-gray-50">
                              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                                ...
                                <div className="flex gap-2 sm:flex-shrink-0">
                                  ...
                                  >
                                    <Edit className="w-4 h-4 sm:mr-1" /> <span>{getTranslation(managingUser.language || 'en', 'edit')}</span>
                                  </Button>
                                  <Button
                                    variant="destructive"
                                    size="sm"
      ...
      <Dialog open={shopProductDialogOpen} onOpenChange={setShopProductDialogOpen}>
        <DialogContent className="sm:max-w-[600px]">
          ...
          <form onSubmit={handleShopProductSubmit} className="space-y-4">
            <div>
              <Label htmlFor="shop-name">{t('productName') || 'Product Name'} *</Label>
              <Input
                id="shop-name"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="shop-price">{t('price')} (EGP) *</Label>
                <Input
                  id="shop-price"
                  type="number"
                  step="0.01"
                  min="0"
                  value={shopProductFormData.price}
                  onChange={(e) => setShopProductFormData({...shopProductFormData, price: parseFloat(e.target.value) || 0})}
                  placeholder="0.00"
                  required
                />
              </div>
              <div>
                <Label htmlFor="shop-stock">{t('stock') || 'Stock'} *</Label>
                <Input
                  id="shop-stock"
                  type="number"
                  min="0"
                  value={shopProductFormData.stock}
                  onChange={(e) => setShopProductFormData({...shopProductFormData, stock: parseInt(e.target.value) || 0})}
                  placeholder="0"
                  required
                />
              </div>
              ...
              <Card>
                ...
                <CardContent>
                  {detailViewUserPlansLoading ? (
                    <div className="text-sm text-gray-500">Loading nutrition plan...</div>
                  ) : (() => {
