...
import { User as UserIcon, Video as VideoIcon, ExternalLink, Plus, Trash2, Edit, FileBadge2, Eye, Clock, Activity, Sparkles, Info, ShoppingBag, Dumbbell, UtensilsCrossed, Target, TrendingUp, CalendarDays, Filter, X, Megaphone } from 'lucide-react';
...
export default function CoachPage() {
  ...
  const { data: myCourses = [], isLoading: coursesLoading } = useQuery({
    queryKey: ['/api/coach/courses'],
    queryFn: async () => {
      const res = await apiRequest('GET', '/api/courses?instructorId=' + user?.id);
      return res.json();
    }
  });
  ...
  const coachEditUserMutation = useMutation({
    mutationFn: async ({ userId, userData }: { userId: number, userData: any }) => {
      const res = await apiRequest('PATCH', `/api/coach/users/${userId}`, userData);
      if (!res.ok) {
    ...
    onSuccess: (updated) => {
      ...
      toast({ title: t('userUpdatedSuccessfully') });
    },
    onError: (e: any) => {
                  ...
                  <div className="space-y-2">
                    {myUsers
                      .filter((u) => {
                        ...
                        return full.includes(q) || contact.includes(q) || email.includes(q);
                      })
                      .map((u) => (
                        <div
                          key={u.id}
                        ...
                        >
                          <div className="flex flex-wrap items-start justify-between gap-2">
                            <div className="flex-1 min-w-0 cursor-pointer" onClick={() => {
                              setSelectedUser(u);
                              setSubscriptionType(u.subscriptionType || '');
                              setSubscriptionStartDate(u.subscriptionStartDate ? new Date(u.subscriptionStartDate).toISOString().slice(0,10) : '');
                            }}>
                              <div className="font-medium">{u.firstName} {u.lastName}</div>
                              {(u as any).email && (
                                <div className="text-xs text-gray-500 truncate">✉ {(u as any).email}</div>
                              )}
                    ...
                    {myUsers.length === 0 && (
                      <div className="text-sm text-gray-500">{t('noUsersAssignedYet') || 'No users assigned yet.'}</div>
                    )}
                  </div>
                )}
              </CardContent>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Quick overview of selected trainee</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {enrolledUsersLoading ? (
                      ...
                      <div className="text-sm text-gray-500">{t('noEnrolledUsers') || 'No users enrolled in this course'}</div>
                    ) : (
      ...
      <Dialog open={editUserDialogOpen} onOpenChange={setEditUserDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>{t('updateTraineeInformation')}</DialogDescription>
          </DialogHeader>
          <form onSubmit={(e) => {
            e.preventDefault();
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="editFirstName" value={editUserFormData.firstName} onChange={(e) => setEditUserFormData({ ...editUserFormData, firstName: e.target.value })} required />
              </div>
              <div>
                <Label htmlFor="editLastName">{t('lastName')}</Label>
                <Input id="editLastName" value={editUserFormData.lastName} onChange={(e) => setEditUserFormData({ ...editUserFormData, lastName: e.target.value })} required />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <Input id="editWhatsapp" value={editUserFormData.whatsappWithCode} onChange={(e) => setEditUserFormData({ ...editUserFormData, whatsappWithCode: e.target.value })} />
            </div>
            <div className="flex gap-2">
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {dialogType === 'nutrition' ? (
            <form onSubmit={(e) => {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="calories" name="calories" type="number" defaultValue={editingPlan?.goals?.calories || ''} />
                </div>
                <div>
                  <Label htmlFor="protein">{t('proteinG')}</Label>
                  <Input id="protein" name="protein" type="number" defaultValue={editingPlan?.goals?.protein || ''} />
                </div>
                <div>
                  <Label htmlFor="carbs">{t('carbsG')}</Label>
                  <Input id="carbs" name="carbs" type="number" defaultValue={editingPlan?.goals?.carbs || ''} />
                </div>
                <div>
                  <Label htmlFor="fat">{t('fatG')}</Label>
                  <Input id="fat" name="fat" type="number" defaultValue={editingPlan?.goals?.fat || ''} />
                </div>
              </div>
              <div>
            ...
            <form onSubmit={(e) => {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="workoutDays">{t('workoutDaysPerWeek')}</Label>
                  <Input id="workoutDays" name="workoutDays" type="number" min="1" max="7" defaultValue={editingPlan?.goals?.workoutDays || 3} />
                </div>
                <div>
    ...
    <TabsContent value="plans">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Latest nutrition plan with macros, meals, and tips</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Dialog open={quickAddOpen} onOpenChange={setQuickAddOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="workoutText">{t('weeklyWorkoutSchedule')}</Label>
              <Textarea
                id="workoutText"
...
function UserSubscriptionSection({ user, subscriptionType, setSubscriptionType, subscriptionStartDate, setSubscriptionStartDate }: {
  user: any;
  subscriptionType: string;
  setSubscriptionType: (v: string) => void;
  subscriptionStartDate: string;
  setSubscriptionStartDate: (v: string) => void;
}) {
  const { t } = useLanguage();
  ...
  const endDatePreview = useMemo(() => {
    ...
    try {
      const formatted = subscriptionType.endsWith('_months') ? subscriptionType : `${subscriptionType}_months`;
      const start = subscriptionStartDate ? new Date(subscriptionStartDate) : new Date();
      const end = calculateSubscriptionEndDate(start, formatted);
      return end.toLocaleDateString();
    } catch { return ''; }
  }, [subscriptionType, subscriptionStartDate]);
    ...
    <TabsContent value="subscription">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Status, dates, and update controls</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <form onSubmit={(e) => {
            ...
            updateSubscription.mutate({ userId: user.id, subscriptionData: data });
          }} className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
            ...
            <div className="text-xs text-gray-500">{t('endDate')}: {endDatePreview || '—'}</div>
            <Button type="submit" disabled={updateSubscription.isPending}>
              {updateSubscription.isPending ? t('updating') : t('updateSubscription')}
            </Button>
          </form>
        </CardContent>
      </Card>
    ...
    <div className="flex flex-wrap items-center gap-2 rounded-lg border bg-slate-50/70 p-2">
      ...
      {showCreateActions && type === 'nutrition' && (
        ...
        >
          ...
          <span className="sm:hidden">{generating ? '...' : 'Gen'}</span>
        </Button>
      )}
      {plan && (
        <>
