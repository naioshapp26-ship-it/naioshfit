...
const ResetPasswordPage: React.FC = () => {
  ...
  const onRequestSubmit = async (values: RequestResetFormValues) => {
    try {
      ...
      if (!response.ok) {
        throw new Error((data.message as string) || 'فشل في إرسال رابط إعادة التعيين');
      }
      ...
      toast({
        ...
        description: (data.message as string) || (language === 'ar' ? 'تحقق من بريدك الإلكتروني' : 'Check your email'),
      });
          ...
          {resetToken ? (
            ...
            <>
              ...
              <Collapsible
                ...
                onOpenChange={(open) => {
                  setShowPinFallback(open);
                  if (open) {
                    const requestEmail = requestForm.getValues('email')?.trim();
                    if (requestEmail && !pinForm.getValues('email')?.trim()) {
                      pinForm.setValue('email', requestEmail);
                    }
                  }
                }}
                className="mt-6"
