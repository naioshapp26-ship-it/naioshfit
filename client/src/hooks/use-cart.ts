...
export function useCart() {
  ...
  const { t } = useLanguage();
  const { user } = useAuth();
  const canAccessCart = Boolean(user) && !isGuestUser(user);

  const assertGuestCanMutate = () => {
    if (!user) {
      throw new Error(t('loginRequired') || 'Please login to continue');
    }
    if (isGuestUser(user)) {
  ...
  return {
    cart: canAccessCart ? cartQuery.data : EMPTY_CART,
    isLoading: canAccessCart && cartQuery.isLoading,
