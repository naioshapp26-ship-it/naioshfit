...
const BillingPage = createLazyRoute(() => import("@/pages/billing"));
...
const AppRoutes = () => {
  ...
  }, [user, isClient, isGuest, location]);
  ...
  const isCartPage = normalizedPath === '/cart';
  ...
  const showPublicFloatingActions =
    ...
    isCartPage ||
    isAdsPage;
  const showPublicAnnouncementBar = isHeroPage || isBlogPage || isCoursesPage || isStorePage || isCartPage || isAdsPage || (!user && isHomePage);
  ...
  if (!user && !isHomePage && !isHeroPage && !isAuthPage && !isLoginPage && !isSignupPage && !isSaasSignupPage && !isResetPage && !isPrivacyPage && !isTosPage && !isBlogPage && !isCoursesPage && !isStorePage && !isCartPage && !isAdsPage && !hasStoredUser) {
  ...
  if (!user && (isHomePage || isHeroPage || isAuthPage || isLoginPage || isSignupPage || isSaasSignupPage || isResetPage || isPrivacyPage || isTosPage || isBlogPage || isCoursesPage || isStorePage || isCartPage || isAdsPage || !hasStoredUser)) {
    return (
      <>
        ...
        {showPublicTopNav && <PublicTopNav includeSpacer={!(isHeroPage || isHomePage)} hasAnnouncementBar={showPublicAnnouncementBar} />}
        <Switch>
          <Route path="/">
            {() => <Home />}
          </Route>
  ...
  const AuthLoginRoute = () => {
    ...
    React.useEffect(() => {
      if (authUser && !isGuestUser(authUser)) {
        navigate(isPlatformAdminRole(authUser.role) ? '/admin' : '/dashboard', { replace: true });
      }
    }, [authUser, navigate]);
    ...
    <>
      ...
      <Layout>
        <Switch>
          ...
          <Route path="/billing" component={isGuest ? GuestBlockedRoute : BillingPage} />
          <Route path="/profile" component={Profile} />
          <Route path="/auth" component={AuthLoginRoute} />
          <Route path="/login" component={AuthLoginRoute} />
          <Route path="/signup">
            {() => {
              const { user: authUser } = useAuth();
              const [, navigate] = useLocation();
